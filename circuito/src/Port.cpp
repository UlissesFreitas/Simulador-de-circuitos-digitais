...
{
    ///Port_NOT *prov = new Port_NOT(*this);
    ///return prov;
...
{
    Port_AND *prov = new Port_AND(*this);
    return prov;
}
...
void Port_AND::simular(const std::vector<bool3S> &in_port)
{
    ...
    if(getNumInputs() == in_port.size()){
        for(unsigned i=1; i<getNumInputs(); i++){
            if(out_port == bool3S::FALSE) break;
...
{
    Port_NAND *prov = new Port_NAND(*this);
    return prov;
}
...
{
    Port_NOR *prov = new Port_NOR(*this);
    return prov;
}
...
{
    Port_XOR *prov = new Port_XOR(*this);
    return prov;
}
...
{
    Port_NXOR *prov = new Port_NXOR(*this);
    return prov;
}
